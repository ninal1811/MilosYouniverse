...
constexpr char V_SHADER_PATH[] = "shaders/vertex_textured.glsl",
               ...
               V_DARK_PATH[] = "shaders/vertex_lit.glsl",
               F_DARK_PATH[] = "shaders/fragment_lit.glsl";
...
ShaderProgram g_shader_program2;
...
void initialise() {
    ...
    g_display_window = SDL_CreateWindow("Milo's Youniverse", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    g_shader_program2.load(V_DARK_PATH, F_DARK_PATH);
    ...
    g_shader_program2.set_projection_matrix(g_projection_matrix);
    g_shader_program2.set_view_matrix(g_view_matrix);
...
void update() {
    ...
    while (delta_time >= FIXED_TIMESTEP) {
        ...
        if (g_is_colliding_bottom == false && g_current_scene->get_state().player->get_collided_bottom())
    ...
    if (player_x > LEVEL1_LEFT_EDGE) {
        g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-player_x, -player_y, 0));
    } else {
        g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-LEVEL1_LEFT_EDGE, -player_y, 0));
    }
    
    if (g_current_scene == g_menu && g_current_scene->get_state().player->get_position().y < -20.0f) {
...
void render2() {
    ...
    g_shader_program2.set_view_matrix(g_view_matrix);
    g_shader_program2.set_light_position_matrix(g_current_scene->get_state().player->get_position());
    g_current_scene->render(&g_shader_program2);
